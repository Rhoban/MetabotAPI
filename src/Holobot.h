...
public:
  ...
  float acc_x, acc_y, acc_z;
	float odometry_x, odometry_y, odometry_yaw;
	float battery1, battery2;
  ...
  float get_opt(int i); /* sensors are identified from 0 to 4, from right to
                           left, viewed from up */
  std::vector<float> get_opts();
  /* yaw, relative to the start in degree, in trigonometric way from the upper
   * view */
  float get_yaw(); /* the yaw computed from the gyro, more precise than the
                      magnetic yaw, but drift (several degree/mn) */
  /* set the current yaw to be the yaw 0.0 deg (as yaw is computed from the
   * gyro, it is relative */
  void reset_yaw();
   ...
   * the way (positive value) make the robot turn in the trigonometric way (from
   * up view) */
  float get_wheel_speeds(int id);

  /* TODO: unités ? */
  void control(float dx, float dy, float turn);
  /* speed: mm/s ; direction : deg, anti trigo from the direction of optic
   * sensors */
